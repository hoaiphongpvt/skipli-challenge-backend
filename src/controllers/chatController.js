...
exports.createRoomChat = async (req, res) => {
    ...
    try {
        const existingRoom = await db.collection('chat_room').where('studentPhone', '==', studentPhone).where('instructorPhone', '==', instructorPhone).limit(1).get()
        if(!existingRoom.empty) {
            return success(res, { roomId: existingRoom.docs[0].id }, 200, 'Room chat already exist')
        }
        ...
        return success(res, {
            roomId: roomRef.id
        }, 201, 'Room chat created successfully')
    } catch (err) {
